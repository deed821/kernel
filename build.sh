...
ARCH=arm64
BUILD_CROSS_COMPILE=~/android/github/aarch64-cortex_a53-linux-gnueabi/bin/aarch64-cortex_a53-linux-gnueabi-
...
if [ $MODEL == "herolte" ]; then
	cp -rf $RDIR/build/ramdisk/g930x/* $RDIR/.work/ramdisk
	cp -rf $RDIR/build/ramdisk/base/* $RDIR/.work/ramdisk
elif [ $MODEL == "hero2lte" ]; then
	cp -rf $RDIR/build/ramdisk/g935x/* $RDIR/.work/ramdisk
	cp -rf $RDIR/build/ramdisk/base/* $RDIR/.work/ramdisk
fi
...
mv -f $RDIR/output/build.log $RDIR/output/build-g930f.log
ZIP_DATE=`date +%Y%m%d`
ZIP_NAME=$KERNELNAME.G930x.v$VERSION_NUMBER.$ZIP_DATE.zip
...
ZIP_DATE=`date +%Y%m%d`
ZIP_NAME=$KERNELNAME.G935x.v$VERSION_NUMBER.$ZIP_DATE.zip
ZIP_FILE_TARGET=$ZIP_FILE_DIR/$ZIP_NAME
FUNC_BUILD_ZIP
[ -d "$RDIR/.work_zip" ] && rm -rf $RDIR/.work_zip
END_TIME=`date +%s`
